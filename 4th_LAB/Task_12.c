...
int main()
{
	int i = 0;
	float a = 0;
	float M[N]; // = { 0.1, -0.154, 10, 100,-250.123, -222, -0.532, 1234, 0, 25.124 , 213, 0.12, -0.12, 666, 0.58123 }; // Ввод с помощью присваивания
	do 
	{
		float number = 0;
		printf("Enter %d %s",i+1,"element: ");
		scanf_s("%e", &number); // Ввод массива с клавиатуры
		M[i] = number;
		i++;
	} while (i < N);
	printf("Non-Reverse: ");
